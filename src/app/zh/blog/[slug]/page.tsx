...
export async function generateStaticParams() {
  return getSortedPosts("zh").map((post) => ({
...
export async function generateMetadata({
  ...
  const post = findPostBySlug("zh", slug);
...
export default async function BlogPostPage({
  ...
  const jsonLdContent = JSON.stringify({
    ...
    image: post.image
      ? `${DATA.url}${post.image}`
      : `${DATA.url}/zh/blog/${slug}/opengraph-image`,
    url: `${DATA.url}/zh/blog/${slug}`,
    author: {
      "@type": "Person",
      name: DATA.name,
    ...
    <section id="blog">
      ...
      <nav className="mt-12 pt-8 max-w-2xl">
        <div className="flex flex-col sm:flex-row justify-between gap-4">
          {previousPost ? (
            <Link
              href={`/zh/blog/${getPostSlug(previousPost._meta.path)}`}
          ...
          {nextPost ? (
            <Link
              href={`/zh/blog/${getPostSlug(nextPost._meta.path)}`}
